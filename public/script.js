...
let latestEntries = [];      // 마지막에 로드한 entries 저장
let resizeTimer = null;
let resizeListenerAttached = false;

// 말풍선 레이아웃 설정
const ROW_PATTERN = [2, 3, 2, 3, 2, 3]; // bottom -> top
const MAX_BUBBLE_COUNT = ROW_PATTERN.reduce((sum, count) => sum + count, 0);
...
async function loadEntries() {
    try {
        ...
        latestEntries = entries.slice(0, MAX_BUBBLE_COUNT);   // 전역에 저장
...
// 위치가 겹치는지 확인하는 함수
function isPositionOverlapping(x, y, positions, minDist) {
...
// 기존 말풍선 위치 수집
function getExistingBubblePositions() {
    const positions = [];
    const bubbles = floatingBubbles.querySelectorAll('.floating-bubble');
...
function buildRowLayout(totalCount) {
    ...
    let patternIndex = 0;
    while (remaining > 0 && patternIndex < ROW_PATTERN.length) {
        const capacity = ROW_PATTERN[patternIndex];
        const rowCount = Math.min(capacity, remaining);
        layout.push(rowCount);
        remaining -= rowCount;
        patternIndex++;
    }
    return layout;
...
function addSingleBubble(entry) {
    ...
    const limeGreenVariations = [
        { bg: 'rgba(144, 238, 144, 0.75)', border: 'rgba(50, 205, 50, 0.7)' },   // 더 투명하게
    ...
    if (existingCount >= MAX_BUBBLE_COUNT) {
        const allBubbles = Array.from(existingBubblesList);
        allBubbles.sort((a, b) => {
...
function createFloatingBubbles(entries) {
    ...
    const numBubbles = selectedEntries.length;
    ...
    selectedEntries.forEach((entry, index) => {
        setTimeout(() => {
            ...
            const randomSize = 1.0; // 크기 통일 (겹침 방지)
            const colorSet = limeGreenVariations[Math.floor(Math.random() * limeGreenVariations.length)];
            const randomDelay = index * 0.1; // 순차적으로 나타나기
            const randomDuration = 6; // 애니메이션 속도 통일
